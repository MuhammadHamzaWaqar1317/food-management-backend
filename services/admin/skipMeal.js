...
exports.deleteEmployeeMealSkip = async (req, res) => {
  ...
  try {
    let { employeeId, skipDateId } = req.query;

    console.log(employeeId, skipDateId);
    const getUser = await employeeMealSubscription.findOne({ employeeId });

    const newLunchDates = getUser.lunch.filter(
      (dates) => skipDateId != dates._id
    );

    console.log(newLunchDates, skipDateId);

    const newDinnerDates = getUser.dinner.filter(
      (dates) => skipDateId != dates._id
    );
